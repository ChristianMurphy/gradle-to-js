...
var KEYWORD_DEF = 'def';
var KEYWORD_IF = 'if';
var KEYWORD_TASK = 'task';
var KEYWORD_APPLY= 'apply';
...
function deepParse(chunk, state, keepFunctionCalls, skipEmptyValues) {
  ...
  for (; state.index < chunkLength; state.index++) {
    ...
    if (character === CHAR_NEWLINE) {
      ...
      // TODO: IS trimWrappingQuotes working???
      if (tempString || (currentKey && !skipEmptyValues)) {
        addValueToStructure(out, currentKey, trimWrappingQuotes(tempString).trim());
    ...
    if (character === CHAR_BLOCK_START) {
      ...
      if (isKeyword(tempString)) {
        if (tempString === KEYWORD_DEF || tempString === KEYWORD_TASK || tempString === KEYWORD_APPLY) {
      ...
      if (!tempString && (isDelimiter(character) || character == CHAR_EQUALS)) {
        continue;
      }
...
function isKeyword(string) {
  return string === KEYWORD_DEF || string === KEYWORD_IF || string === KEYWORD_TASK;
}
